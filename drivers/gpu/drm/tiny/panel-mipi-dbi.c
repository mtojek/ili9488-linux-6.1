...
static int panel_mipi_dbi_get_format(struct device *dev, u32 *formats, unsigned int *bpp)
{
	...
	for (i = 0; i < ARRAY_SIZE(panel_mipi_dbi_formats); i++) {
		...
		*bpp = format->bpp;
		...
		dev_info(dev, "MARCIN Format: %s, bpp = %d\n", format->name, *bpp);
		return 0;
	}

	dev_err(dev, "MARCIN Pixel format is not supported: '%s'\n", format_name);
	return -EINVAL;
}
...
static const struct drm_driver panel_mipi_dbi_driver = {
	...
	.name			= "panel-mipi-dbi",
	.desc			= "MIPI DBI compatible display panel (custom)",
	.date			= "20220103",
	.major			= 1,
	.minor			= 0,
};
...
static int panel_mipi_dbi_spi_probe(struct spi_device *spi)
{
	...
	dev_info(dev, "MARCIN probe info\n");
...
MODULE_DESCRIPTION("MIPI DBI compatible display panel driver (custom)");
